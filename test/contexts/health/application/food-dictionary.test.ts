...
describe("searchFoodDictionary", () => {
  it("excludes non-matching items and returns items matching a name substring", async () => {
    repo.seed({ ownerUserId: null, name: "香蕉/1根", carbG: 30, proteinG: 0, fatG: 0, sugarG: 0, fiberG: 0, kcal: 120, staple: 0, meat: 0, fruit: 2, veg: 0 });
    repo.seed({ ownerUserId: null, name: "飯/1碗", carbG: 60, proteinG: 0, fatG: 0, sugarG: 0, fiberG: 0, kcal: 240, staple: 4, meat: 0, fruit: 0, veg: 0 });
  ...
  it("returns shared items and the searching user's own custom items, excluding another user's custom items", async () => {
    repo.seed({ ownerUserId: null, name: "自訂菜/1份", carbG: 10, proteinG: 0, fatG: 0, sugarG: 0, fiberG: 0, kcal: 40, staple: 0, meat: 0, fruit: 0, veg: 1 });
...
describe("favoriteFoodItem / unfavoriteFoodItem / listFavoriteFoodItems", () => {
  it("marks an item as a favorite and lists it", async () => {
    const item = repo.seed({ ownerUserId: null, name: "豆漿/1杯", carbG: 8, proteinG: 7, fatG: 4, sugarG: 4, fiberG: 0, kcal: 100, staple: 0, meat: 1, fruit: 0, veg: 0 });
