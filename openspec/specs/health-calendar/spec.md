...
The system SHALL expose, over an authenticated HTTP API, a monthly summary `GET /api/health-calendar?month=YYYY-MM` returning `{ year, month, logged_days, days_elapsed, logging_rate, diet_adherence_rate }`. `days_elapsed` SHALL be the day-of-month of "today" for the current month, the number of days in the month for a past month, and 0 for a future month, where "today" is an optional client-supplied `today=YYYY-MM-DD` query (so a user's local calendar day is used rather than the server's UTC day), falling back to the server's UTC date when absent; a malformed `today` SHALL be rejected with 400. `logging_rate` SHALL be `round(100 × logged_days_count / days_elapsed)` and `diet_adherence_rate` the rounded percentage of elapsed days that met the diet target (the day has a target and every category is fully logged); both SHALL be null when `days_elapsed` is 0. A missing or malformed `month` SHALL be rejected with 400, and an unauthenticated request with 401.
