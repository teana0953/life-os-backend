...
- **Subrequest ceiling exceeded on the Free plan** → the whole endpoint would fail, which is the failure mode this change most needs to avoid. Mitigation: D6's counting task, done before merge, with the count asserted in a test so adding a section later trips the assertion rather than production.
