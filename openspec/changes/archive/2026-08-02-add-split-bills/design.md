...
**但「呼叫者本人是不是參與者」這一條要看的是有沒有真的利害關係,不是名字有沒有出現。** code review 抓到:`payer=B, shares={A:0, C:1000}` 這種寫法讓 A 名列 shares 卻不欠也不被欠,存下來就是「C 欠 B 1000」而 A 實質不在裡面——正是這條規則要擋的偽造。所以判準是:**呼叫者必須是付款人,或持有一份金額 > 0 的 share**。其他參與者的 share 為 0 仍然合法(同桌但這次不用分攤是真實情境,而且擋不擋它跟偽造無關——呼叫者自己還是得真的在這筆帳裡)。已知極限:**呼叫者給自己 1 個最小單位就通過**,這是接受的——這條規則要的是「有實質利害關係」,1 個單位就是利害關係,再加門檻只會傷到真實的小額分攤。
...
| 動作 | 誰可以 |
|---|---|
| 建立分帳 | **呼叫者本人必須有實質利害關係**(付款人,或持有一份**金額 > 0** 的 share);**群組支出**(`group_id` 不為 null):付款人與每個持有 share 的人**都必須是該群組的成員**;**一對一支出**(`group_id` 為 null):都必須是呼叫者本人或呼叫者的好友;否則 400 |
| 編輯/刪除分帳 | `created_by_user_id` 或 `payer_user_id`。**其他分擔人不能改**——他們沒付錢也沒建立,改了等於單方面改別人的帳 |
| 加入群組成員 | 群組現有成員,且被加的人是**加入者的好友**;不能把陌生人拉進來。**被加的人已經是成員 → 400**(不是靠 unique constraint 炸成 500:前端連點兩下「加入」是很正常的事) |
...
**編輯要重跑建立時的全部驗證,包含那兩條授權規則。**(proposal review 兩輪都抓到這裡)`PATCH` 不是只換幾個欄位:金額>0、幣別、day、shares 非空無重複、參與者>1、總和相等、每份非負、每個人是好友或同群組成員、**呼叫者編輯後仍必須有實質利害關係**(share 歸零跟整份拿掉一樣擋)、**`payer_user_id` 一樣要驗**——一條都不能少。
...
- 兩個同時給 → 400(語意衝突,不猜)。
- 兩個都沒給 → 我參與的全部支出,**「參與」用的是同一個定義**——付款人、持有 share 的人,**或該群組的成員**。少了最後那段,同一筆群組支出在 `?group_id=` 看得到、在無篩選清單卻不見了,兩份清單對「我看得到什麼」講出兩種答案。
- **參與條件一定要寫進 SQL WHERE**(對 `split_share` 與 `expense_group_member` 用 `EXISTS`),不得先撈全部再在記憶體篩。
- **群組成員資格一組群一次查,不是一列一次**:use case 那層的逐列複查若對每一列都去問 `membersAmong`,一個 200 筆的群組清單就是 200 趟 neon-http。

## 驗證與邊界(每條都要測)

- `amount <= 0` → 400;`shares` 為空 → 400;同一人在 shares 出現兩次 → 400(unique 也擋,但要回可讀的錯)
- **金額必須是 JSON number**:`"100"`、`"1e3"` 這種數字字串一律 400,不做隱式轉型。共用的 `requireFiniteNumber` 會轉型,而其他 context 已經依賴那個行為,所以**這條在 `routes/split.ts` 自己擋**,不去改共用 helper 的合約。整數判斷仍然留在 domain(錢的規則歸 domain)
- `sum(shares) != amount` → 400,錯誤訊息要說出差多少(使用者才知道要改哪裡)
- 幣別非三碼大寫 → 400;`day` 非 `YYYY-MM-DD` → 400
- **`amount` 超過 2147483647 → 400**。兩個金額欄位都是 Postgres `integer`(4 byte);沒有這條的話,值會通過所有 application 檢查、由 Postgres 丟 22003,對使用者變成 **500**——一個 bad request 被報成伺服器故障
- **均分金額小於參與人數時,錯誤要說「金額太小」而不是「你不是參與者」**。`equalSplit` 會把 canonical uuid 排序末端的人分到 0;那個人剛好是呼叫者時,stake 規則會擋下來,但回 `not_a_participant` 等於告訴他「你不在這筆自己明明列了自己的支出裡」,他會往錯的方向找
