...
export async function deleteExpense(repository: SplitExpenseRepository, callerUserId: string, expenseId: string): Promise<void> {
  ...
  const deleted = await repository.delete(expenseId);
